...
class NotificationActionHandler implements INotificationActionHandler {
  ...
  Future<void> executeActions(List<NotificationAction> actions) async {
    ...
    for (final action in actions.whereType<CancelNotificationAction>()) {
      ...
      if (untrackActions.any((ua) => ua.videoId == action.videoId && (ua.liveNotificationId == action.notificationId || ua.reminderNotificationId == action.notificationId ))) continue;
    ...
    if (videosToWrite.isNotEmpty) {
        _logger.debug("ActionHandler: Preparing final cache state for ${videosToWrite.length} videos...");
        ...
        for(final videoId in videosToWrite) {
            ...
            if (updatesByVideo.containsKey(videoId)) {
                for (final updateAction in updatesByVideo[videoId]!) {
                   workingCompanion = workingCompanion.copyWith( 
                       ...
                       thumbnailUrl: updateAction.companion.thumbnailUrl.present ? updateAction.companion.thumbnailUrl : workingCompanion.thumbnailUrl,
                        ...
                        mentionedChannelIds: updateAction.companion.mentionedChannelIds.present ? updateAction.companion.mentionedChannelIds : workingCompanion.mentionedChannelIds,
                       ...
                       channelAvatarUrl: updateAction.companion.channelAvatarUrl.present ? updateAction.companion.channelAvatarUrl : workingCompanion.channelAvatarUrl,
                       isPendingNewMediaNotification: updateAction.companion.isPendingNewMediaNotification.present ? updateAction.companion.isPendingNewMediaNotification : workingCompanion.isPendingNewMediaNotification,
                       lastSeenTimestamp: updateAction.companion.lastSeenTimestamp.present ? updateAction.companion.lastSeenTimestamp : workingCompanion.lastSeenTimestamp,
                       scheduledLiveNotificationId: updateAction.companion.scheduledLiveNotificationId.present ? updateAction.companion.scheduledLiveNotificationId : workingCompanion.scheduledLiveNotificationId,
                        lastLiveNotificationSentTime: updateAction.companion.lastLiveNotificationSentTime.present ? updateAction.companion.lastLiveNotificationSentTime : workingCompanion.lastLiveNotificationSentTime,
                        scheduledReminderNotificationId: updateAction.companion.scheduledReminderNotificationId.present ? updateAction.companion.scheduledReminderNotificationId : workingCompanion.scheduledReminderNotificationId,
                        scheduledReminderTime: updateAction.companion.scheduledReminderTime.present ? updateAction.companion.scheduledReminderTime : workingCompanion.scheduledReminderTime,
                       userDismissedAt: updateAction.companion.userDismissedAt.present ? updateAction.companion.userDismissedAt : workingCompanion.userDismissedAt,
                        sentMentionTargetIds: updateAction.companion.sentMentionTargetIds.present ? updateAction.companion.sentMentionTargetIds : workingCompanion.sentMentionTargetIds,
                   );
                }
            }
             ...
             if (scheduledIdsMap.containsKey(videoId)) {
                 final scheduledIds = scheduledIdsMap[videoId]!;
                  if (scheduledIds.liveId != null && workingCompanion.scheduledLiveNotificationId.present && workingCompanion.scheduledLiveNotificationId.value == -1) {
                     workingCompanion = workingCompanion.copyWith(scheduledLiveNotificationId: Value(scheduledIds.liveId));
                  }
                  if (scheduledIds.reminderId != null && workingCompanion.scheduledReminderNotificationId.present && workingCompanion.scheduledReminderNotificationId.value == -1) {
            ...
            if (currentVideo == null) {
                workingCompanion = workingCompanion.copyWith(
                    ...
                    lastSeenTimestamp: workingCompanion.lastSeenTimestamp.present ? workingCompanion.lastSeenTimestamp : Value(DateTime.now().millisecondsSinceEpoch),
        ...
        _logger.debug("ActionHandler: Persisting final cache state for ${finalCompanions.length} videos...");
        ...
        _logger.info("ActionHandler: Finished applying final cache updates.");
